...
// Bundled Godot 4 API docs — generated by `npm run build:docs` from
// godotengine/godot's doc/classes/*.xml and shipped in the npm tarball
// at dist/data/godot-docs.json.
interface ClassDoc {
  name: string;
  description: string;
  inherits: string;
...
function loadDocsIndex(): Map<string, ClassDoc> {
  // dist layout (after `tsc` build): dist/tools/docs-search.js + dist/data/godot-docs.json
  // src layout (when running tests via vitest): src/tools/docs-search.ts + src/data/godot-docs.json
  const here = dirname(fileURLToPath(import.meta.url));
  const candidates = [
    resolve(here, "..", "data", "godot-docs.json"),
    resolve(here, "..", "..", "src", "data", "godot-docs.json"),
  ];

  for (const candidate of candidates) {
    try {
      const raw = readFileSync(candidate, "utf-8");
      const parsed = JSON.parse(raw) as Record<string, ClassDoc>;
  ...
  // Fall back to empty — server still works for the migration map.
  // stderr is visible to MCP clients but doesn't pollute the protocol.
  console.error(
    "[godot-forge] godot-docs.json not found; class lookups will return migration hints only. Run `npm run build:docs` to populate."
  );
  return new Map();
}

const docsIndex: Map<string, ClassDoc> = loadDocsIndex();
...
export function registerDocsSearch(server: McpServer, _ctx: ServerContext): void {
  server.tool(
    ...
    async (args) => {
      ...
      const indexNote =
        docsIndex.size === 0
          ? " (Note: bundled API docs were not found; only the Godot 3→4 migration mapping is active.)"
          : ` Searched ${docsIndex.size} Godot 4 classes.`;
      return {
        content: [
          {
            type: "text",
            text: `No results found for "${query}".${indexNote}`,
          },
        ],
      };
    }
